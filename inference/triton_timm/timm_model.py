...
def create_model(
    ...
    image_encoder_path = os.path.join(triton_model_repository_path, friendly_name, "1", "model.onnx")
    ...
    if not os.path.exists(image_encoder_path):
        ...
        generate_timm_config(cfg_path, friendly_name, model_cfg.input_size, model_cfg.num_classes)

        save_library_name(
            os.path.join(triton_model_repository_path, friendly_name), "timm"
        )

        with open(os.path.join(triton_model_repository_path, friendly_name, "data_config.json"), "w") as f:
...
class TritonTimmInferenceClient(TritonModelInferenceClient):
    def __init__(
        ...
        if not self.triton_client.is_model_ready(self.model_nice_name):
            ...
            with open(os.path.join(triton_model_repository_path, self.model_nice_name, "data_config.json"), "r") as f:
